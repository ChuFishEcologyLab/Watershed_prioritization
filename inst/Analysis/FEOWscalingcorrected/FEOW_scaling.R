...
hyb5r = hyb5r%>% group_by(FEOW_ID)%>%
  dplyr::mutate(Prot_feow_scaled=(((Prot_rank_feow-1)*maxrank/max(Prot_rank_feow-1))/maxrank)*(100 - 1) + 1)

maxrank = max(hyb5r$Rest_rank_feow-1)
hyb5r = hyb5r%>% group_by(FEOW_ID)%>%
  dplyr::mutate(Rest_feow_scaled=(((Rest_rank_feow-1)*maxrank/max(Rest_rank_feow-1))/maxrank)*(100 - 1) + 1)

maxrank = max(hyb5r$SAR_rank_feow-1)
hyb5r = hyb5r%>% group_by(FEOW_ID)%>%
  dplyr::mutate(SAR_feow_scaled=(((SAR_rank_feow-1)*maxrank/max(SAR_rank_feow-1))/maxrank)*(100 - 1) + 1)

maxrank = max(hyb5r$AIS_rank_feow-1)
hyb5r = hyb5r%>% group_by(FEOW_ID)%>%
  dplyr::mutate(AIS_feow_scaled=(((AIS_rank_feow-1)*maxrank/max(AIS_rank_feow-1))/maxrank)*(100 - 1) + 1)
