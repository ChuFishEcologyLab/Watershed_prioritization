...
pdata1$name<- rep(c("Watershed stress", "Community change", "Climate change",
                    "Species at risk", "Species richness", "Species rarity"), 8)
...
pdata2$name<- rep(c("Watershed stress", "Community change", "Climate change",
                    "Species at risk", "Species richness", "Species rarity"), 8)
...
pdata3$name<- rep(c("Watershed stress", "Community change", "Climate change",
                    "Species at risk", "Species richness", "Species rarity"), 8)
...
pdata4$name<- rep(c("Watershed stress", "Community change", "Climate change",
                    "Species at risk", "Species richness", "Species rarity"), 8)
...
ggarrange (S2_A,S2_B,
           ...
           ncol = 2, nrow=3, common.legend = TRUE, 
             legend = "bottom") # Write the grid.arrange in the file
dev.off() # Close the file
############
...
A1 = ggplot() +
  ...
  scale_fill_viridis( alpha = 1, end = 0.9, direction = -1, option = "magma",
...
ggarrange (A1,B1,C1,D1, ncol = 2, nrow=2, common.legend = TRUE, 
...
###Comparison plots
##############
pal = c(viridis_pal(option = "magma")(20)[20], 
...
p1 = ggplot()+
  ...
  geom_point(data=data, aes(x=Prot_rank_feow_scaled,
...
p2 = ggplot()+
  geom_rect(data = rects, aes(xmin = xmins, ymin=ymins, xmax=xmaxs, ymax=ymaxs, fill = fills))+
  geom_point(data=data, aes(x=SAR_rank_feow_scaled,
...
s1 = ggplot(data=df, aes(x=threshold*100, y=100-(ratio*100), group = objective, color = objective))+
  ...
  scale_x_continuous(name = "Percentage of watersheds prioritized",
                     limits =c(0, 59),
                     breaks= c(10,20,30,40,50))+
  theme(legend.position = "none",
        panel.grid.minor = element_blank())+
  geom_text(data=mylabels, 
            aes(x=50.5, 
                y=100-(ratio*100),
                label = objective, 
                color = objective),
