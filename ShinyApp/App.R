...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    hr(),
    p(
      strong("Watersheds ranked within ecoregions?"),
      br(),
...
server <- function(input, output, session) {
  ...
  data5 <- readr::read_csv(
    "watershed_prioritization_level5.csv",
    show_col_types = FALSE
  ) |>
    dplyr::select(
      HYBAS_ID, FEOW_ID, WSI_n, FBCI_n, CCI_n, SARI_n, Fish_richness_n, Priority_n
    )
  data6 <- readr::read_csv(
